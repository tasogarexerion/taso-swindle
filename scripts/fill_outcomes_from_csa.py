...
CSA_MOVE_RE = re.compile(r"^[+-](\d{2})(\d{2})([A-Z]{2})$")
...
PROMOTED_CODES = {"TO", "NY", "NK", "NG", "UM", "RY"}
...
def _rank_to_letter(rank: str) -> str:
    n = int(rank)
    return chr(ord("a") + n - 1)


def _to_sq(two: str) -> str:
    return f"{two[0]}{_rank_to_letter(two[1])}"


def csa_to_usi_move(line: str) -> Optional[str]:
    m = CSA_MOVE_RE.match(line.strip())
    if not m:
        return None
    src, dst, piece = m.group(1), m.group(2), m.group(3)
    if src == "00":
        drop = PIECE_DROP_MAP.get(piece)
        if drop is None:
            return None
        return f"{drop}*{_to_sq(dst)}"

    move = f"{_to_sq(src)}{_to_sq(dst)}"
    if piece in PROMOTED_CODES:
        move += "+"
    return move


def _collect_csa_paths(csa: str, csa_dir: str) -> list[Path]:
    paths: list[Path] = []
    raw_file = (csa or "").strip()
    if raw_file:
        p = Path(raw_file)
        if p.is_dir():
            paths.extend(sorted(p.glob("*.csa")))
            paths.extend(sorted(p.glob("*.CSA")))
        elif p.exists():
            paths.append(p)

    raw_dir = (csa_dir or "").strip()
    if raw_dir:
        d = Path(raw_dir)
        if d.exists() and d.is_dir():
            paths.extend(sorted(d.glob("*.csa")))
            paths.extend(sorted(d.glob("*.CSA")))
...
def parse_csa(path: Path) -> CsaGame:
    moves: list[str] = []
    game_id = ""
    start_time = ""
    sente = ""
    ...
    for raw in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if line.startswith("$"):
            ...
            if ":" in body:
                ...
                keyn = _norm(key)
                val = value.strip()
                if keyn in {"game_id", "id", "対局id"} and not game_id:
                    game_id = val
                elif keyn in {"start_time", "開始日時", "date"} and not start_time:
                    start_time = val
            continue
        ...
        usi = csa_to_usi_move(line)
        if usi and USI_MOVE_RE.match(usi):
            moves.append(usi)
...
def _score_game_match(rec: dict[str, Any], game: CsaGame) -> float:
    score = 0.0
...
def _pick_best_game(rec: dict[str, Any], games: list[CsaGame]) -> MatchResult:
    if not games:
        return MatchResult(game=None, source="unmatched", confidence=0.0, candidates=0)

    scored: list[tuple[float, CsaGame]] = []
    for game in games:
...
def fill_records(records: list[dict[str, Any]], games: list[CsaGame]) -> list[dict[str, Any]]:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fill DecisionEvent JSONL with actual_opponent_move/outcome_tag from CSA")
    parser.add_argument("--input", required=True, help="input DecisionEvent JSONL")
    parser.add_argument("--csa", default="", help="csa file path (or directory)")
    parser.add_argument("--csa-dir", default="", help="csa directory (multiple files scanned)")
    ...
    csa_paths = _collect_csa_paths(args.csa, args.csa_dir)
    if not csa_paths:
        raise SystemExit("no csa files found: pass --csa or --csa-dir")

    games = [parse_csa(path) for path in csa_paths]

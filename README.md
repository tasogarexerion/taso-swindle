...
TASO-SWINDLE の「逆転らしさ」は、専用の `nn.bin` で実現しているわけではありません。

- `eval/nn.bin`
  - これはバックエンドエンジンが通常の局面評価を行うための評価関数です
  - つまり、やねうら王側の土台となる通常評価です
- `models/hybrid_weights.json`
  - これは TASO-SWINDLE 側の逆転判断に使う学習済み重みです
  - `MateUrgency`、OnlyMovePressure、ReplyEntropy、Trap、Risk などの特徴量をどれだけ重視するかに効きます
- `taso_swindle/`
  - 実際の逆転選択ロジック本体です
  - backend の `info` を読んで候補手を再評価し、必要なら `bestmove` を差し替えます

要するに、

- `nn.bin` は「将棋エンジンとしての通常評価」
- `hybrid_weights.json` は「逆転専用ラッパーとしての味付け」

という役割分担です。

そのため、逆転モードをきちんと使いたい場合は `nn.bin` だけでなく `models/hybrid_weights.json` も一緒に置く前提になります。
